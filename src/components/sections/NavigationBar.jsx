...
export const NavigationBar = () => {
  ...
  return (
    <>
      ...
      {/* Backdrop - always in DOM, GPU-only opacity transition */}
      <div
        className={`fixed inset-0 z-[100] md:hidden mobile-menu-backdrop ${mobileOpen ? 'open' : ''}`}
        style={{ touchAction: 'none' }}
        onClick={closeMobile}
      />

      {/* Panel - always in DOM, GPU-only transform transition */}
      <div
        className={`fixed inset-y-0 right-0 w-[85%] max-w-[360px] z-[101] md:hidden mobile-menu-panel ${mobileOpen ? 'open' : ''}`}
        style={{ touchAction: 'none', overscrollBehavior: 'none' }}
      >
        <div className="relative h-full flex flex-col px-6 py-5 overflow-hidden">
          <div className="relative z-10 flex items-center justify-between">
            <img src="/images/logo.png" alt="DruCare" className="h-9 w-auto brightness-0 invert" loading="lazy" />
            <button
              onClick={closeMobile}
            ...
            <div className="space-y-1">
              {NAV_LINKS.map((link, i) => (
                <div
                  ...
                  style={{ transitionDelay: mobileOpen ? `${0.18 + i * 0.035}s` : '0s' }}
                >
                  <button
                    onClick={() => scrollTo(link.href)}
                    className="w-full text-start text-2xl font-semibold text-white/90 hover:text-white py-3.5 px-4 rounded-xl transition-colors duration-200 cursor-pointer hover:bg-white/8"
          ...
          <div
            ...
            style={{ transitionDelay: mobileOpen ? '0.38s' : '0s' }}
          >
            <a
              href="https://app.shopix.global/user/drucare"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 w-full rounded-xl min-h-[52px] text-base font-bold cursor-pointer text-white"
              style={{
